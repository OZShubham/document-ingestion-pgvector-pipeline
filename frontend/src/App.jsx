...
const uploadToGCS = async (fileItem) => {
  try {
    ...
    // UPDATED: Direct upload instead of signed URL
    ...
    addNotification('success', `${fileItem.file.name} uploaded (${result.size_mb}MB) - Processing started`);
    
    setRealTimeUpdates(prev => [{
      id: Date.now(),
      message: `Uploaded "${fileItem.file.name}" (${result.size_mb}MB) - Cloud Function triggered`,
      timestamp: new Date().toISOString(),
      type: 'processing'
    }, ...prev.slice(0, 9)]);
