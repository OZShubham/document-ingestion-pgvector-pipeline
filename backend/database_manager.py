...
#     Properly initializes all async objects in the event loop.
#     """
...
#             # CRITICAL FIX: Pass loop parameter explicitly
#             loop = asyncio.get_running_loop()
#             self._connector = Connector(loop=loop)
#             logger.info("✅ Cloud SQL Connector initialized with explicit loop")
...
#             db_instance = os.getenv("DB_INSTANCE", Config.DB_INSTANCE)
#             db_password = os.getenv("DB_PASSWORD", Config.DB_PASSWORD)
#             db_user = os.getenv("DB_USER", "postgres")

#             logger.info(f"🔌 Initializing database connection pool...")
#             logger.info(f"   Instance: {Config.PROJECT_ID}:{Config.DB_REGION}:{db_instance}")
...
#                 try:
#                     # Make sure we're using the same connector
